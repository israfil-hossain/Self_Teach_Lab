  ...
  <div class="panel-body">
    ...
    <form method="POST" action="{{ route('instructor.profile.save') }}" id="profileForm" enctype="multipart/form-data">
      ...
      <div class="row">
        ...
        <div class="form-group col-md-4">
            <label class="form-control-label">Bkash ID <span class="required">*</span></label>
            <input type="text" class="form-control" name="Bkash_id" 
                placeholder="Bkash ID" value="{{ $instructor->Bkash_id }}" />
                @if ($errors->has('Bkash_id'))
                    <label class="error" for="Bkash_id">{{ $errors->first('Bkash_id') }}</label>
                @endif
        </div>
